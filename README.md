...
서울 열린데이터광장에서 제공한 샘플URL을 통해 브라우저에서 호출했을 땐 올바른 데이터가 반환되었지만,  
매개변수로 받은 지하철역 명을 URL에 넣어 호출하니 원하는 대로 작동하지 않았다. (JSON데이터는 잘 받아왔기 때문에 URL문제라고 판단)  
찾아보니 URL 운영체제마다 일부 문자를 인식하는 방식이 다르기 때문에 특정한 값들은 규칙에 맞게 변환되어야 하고,  
한글은 ASCII코드를 16진수화한 결과를 두자리의 대문자로 나타내어 인코딩을 해야한다는 것을 알게 되었다.
```
String encodeSubwayName = URLEncoder.encode(subwayName, "UTF-8");
```
위 코드를 추가해 지하철역 명을 인코딩해주었고, 
