...
<details>
  ...
  * @Valid가 적용된 MemberDto 객체에서 유효성 검사 중 에러가 발생하면 error.hasErrors()메서드에서는 true값이 반환되고, 조건문을 사용하여 에러가 발생했을 시 메세지를 출력한 후 작성하던 화면 그대로 있게 코드를 작성하였다.  
  ...
  * 나는 각 Controller에서 검증할 내용이 서로 달랐기 떄문에 Validator 인터페이스를 구현한 클래스를 추상클래스로 만들었다.
